...
class Model(nn.Module):
    def __init__(self, name_model):
        ...
        num_features = self.features_extractor(self.features_extractor.dummy_inputs['input_ids'])['last_hidden_state'].shape[-1]

        self.pooler = MeanPooling()

        self.linear = nn.Linear(num_features, num_features//2)
        self.cls_list = nn.ModuleList([nn.Linear(num_features//2, 1) for i in range(6)])
